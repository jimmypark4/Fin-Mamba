...
class MambaBlocks(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_labels = getattr(config, "num_labels", 3)  # 라벨 수 설정

        # Mamba LM 설정
        self.mamba_config = AutoConfig.from_pretrained("state-spaces/mamba-130m-hf")  # 사전 학습된 Mamba 모델 설정 로드
        self.mamba_config.output_hidden_states = True  # 은닉 상태 출력 설정
        self.mamba_model = AutoModelForCausalLM.from_pretrained(
            "state-spaces/mamba-130m-hf",  # 사전 학습된 모델 로드
        ...
        self.rnn1 = rnn(768, lstm_hidden_size, bidirectional=True)  # 첫 번째 Bi-LSTM 레이어
        self.rnn2 = rnn(lstm_hidden_size, lstm_hidden_size, bidirectional=True)  # 두 번째 Bi-LSTM 레이어

        # MambaBlock들 (config에서 개수 설정)
        num_mamba_blocks = getattr(config, "num_mamba_blocks", 7)  # config에서 MambaBlock 개수 가져오기, 기본값 7
        self.mamba_blocks = nn.ModuleList([
            MambaBlock(d_model=800, d_state=16, d_conv=4, expand=2) for _ in range(num_mamba_blocks)  # 동적으로 MambaBlock 생성
        ])
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # C. MambaBlock 처리 (동적으로 설정된 개수의 블록을 순차적으로 처리)
